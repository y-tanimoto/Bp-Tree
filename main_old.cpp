...
public:
    ...
    // 子ノードを2つ持つノードの生成
    // ノード分割時の親ノードの生成に用いる
    void clear_as_parent_node(const int size, Node* parent_node_p, Node* left_child_node_p, Node* right_child_node_p) {
        // 初期化
        Node(size, parent_node_p);

        // 左子ノードの登録
        m_pointers_array[0] = left_child_node_p;

        // 右子ノードの登録
        m_pointers_array[1] = right_child_node_p;

        // キー値は右子ノードの最小値
        m_keys_array[0] = right_child_node_p->get_min_key_recursive();
...
public:
    ...
    bool add_key(Node* target_node, const int key_to_add, Node* node_p_to_add) {
        ...
        // -> ノードを分割し親ノードに追加
            ...
            // 追加するキーを右ノードに登録
            right_node.add_key(key_to_add, node_p_to_add);
            // コピーした要素を左ノードから削除
            for (int i=left_node_keys; i<target_node->get_total_keys(); i++) {
            ...
            if (add_to_new_root_node) {
                parent_node->clear_as_parent_node(m_node_size, nullptr, target_node, &right_node);
                return true;
            }

            // そうでなければ、既存の親ノードに分割後の右ノードを登録
